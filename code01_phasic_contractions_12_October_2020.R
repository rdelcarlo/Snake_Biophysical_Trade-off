# C41P - Data analysis
#   Protocol 1: Control Four Pulse (C4P) Experiment
#   1st stimulus occurs at 5s (seconds)
#   System delay is 4ms (miliseconds)
#   Pulsewidth is 500.0us (microseconds)
#   Stimulus rate is 0.2 Hz
#   Last stimulus is at 24s
#
# Processing:
# 1. Pulse on if stimulus > 1.0
# 2. Base force = average of prior 1000 readings
# 3. Data of interest - 1500 (150ms) after end of pulse
# 4. Normalized force = (measuredF - baseF)*9.80665/(Muscle Mass in Kgs)
#

setwd("C:/Users/rdelcarlo/Desktop/data_r_script")
#columns in this file read:

# Get Snake Info - Species, Genotype etc.
#Snake/Species/Genotype/MAMU/County/Longitude/Latitude/SVLmm/BodMassg/Sex/Date_Experimented/Date_Collected/Days_in_Between/Rater_(M1)/Rater_(M2)/Rater_(M3)/Rater_(M4)/Rater_(M5)/Rater_(M6)/Rater_(M7)/Rater_(M8)/Rater_(M9)/Rater_(M10)/Rater_(M11)/Rater_(M12)/Rater_(M13)
sinf = read.csv("SnakeInfo-09.30.2020.csv")

# Get Snake Muscle Masses
smm = read.csv("SnakeSkeletalMuscleMasses-9.28.2020.csv")
# reset all missing muscle mass values to -1.0smm
smm[is.na(smm)] <- 0.999

# dname = "C:/Bobby/Data-CSV/p1c4p"
# dname = "C:/Bobby/Data-CSV/p1c4pTest"

dname = "C:/Users/rdelcarlo/Desktop/data_r_script/p1c4p"

setwd(dname)

x = strsplit(dname, "/")

# Write / overwrite Column Headers for output summary file
ofsum = paste("./output/", x[[1]][lengths(x)],  ".csv", sep = "")
hdrs <- c(
  "Species",
  "Genotype",
  "Snake",
  "Muscle",
  "Rater",
  "Mass(g)",
  "Genotype",
  "Pulse",
  "BaseF(N/g)",
  "ContrAmpl",
  "ToMaxF(ms)",
  "To10pct(ms)",
  "To50pct(ms)",
  "10to50pct(ms)",
  "FMaxRateOfChg(N/g/s)",
  "FMinRateOfChg(N/g/s)",
  "ToFChgMax(ms)",
  "ToFChgMin(ms)",
  "DiffFChgMaxToMin(ms)",
  "MAMU",
  "County",
  "Longitude",
  "Latitude",
  "SVLmm",
  "MusMassg",
  "BodMassg",
  "Sex",
  "Date_Experimented",
  "Date_Collected",
  "Days_In_Between"
)
write.table(
  t(hdrs),
  file = ofsum,
  append = FALSE,
  quote = TRUE,
  sep = ",",
  eol = "\n",
  na = "NA",
  dec = ".",
  row.names = FALSE,
  col.names = FALSE
)
# Output Force File
ofF <- "./output/c4p1-force.csv"
ofFhdr <- c(
  "Species",
  "Genotype",
  "MAMU",
  "County",
  "Longitude",
  "Latitude",
  "SVLmm",
  "MusMassg",
  "BodMassg",
  "Sex",
  "Date_Experimented",
  "Date_Collected",
  "Days_In_Between",
  "Pulse",
  "Snake",
  "Muscle",
  "Rater",
  "MuscMassg",
  (1:1500) / 10000
)
write.table(
  t(ofFhdr),
  file = ofF,
  append = FALSE,
  quote = TRUE,
  sep = ",",
  eol = "\n",
  na = "NA",
  dec = ".",
  row.names = FALSE,
  col.names = FALSE
)
# Output Force first derivative File
ofF1d <- "./output/c4p1-force1d.csv"
ofF1dhdr <- c(
  "Species",
  "Genotype",
  "MAMU",
  "County",
  "Longitude",
  "Latitude",
  "SVLmm",
  "sMusMassg",
  "sBodMassg",
  "Sex",
  "Date_Experimented",
  "Date_Collected",
  "Days_In_Between",
  "Pulse",
  "Snake",
  "Muscle",
  "Rater",
  "MuscMassg",
  (1:1500) / 10000 #need to find a cutoff filter instead of downsampling filter
)
write.table(
  t(ofF1dhdr),
  file = ofF1d,
  append = FALSE,
  quote = TRUE,
  sep = ",",
  eol = "\n",
  na = "NA",
  dec = ".",
  row.names = FALSE,
  col.names = FALSE
)

# get all data files
files = list.files(path = ".", pattern = "csv")
q <- strsplit(files, "-")
# snakes <- unlist(lapply(q,'[[',1))
# muscles <- unlist(lapply(q,'[[',2))
# osum <- data.frame(files,snakes,muscles)

fC4P <- paste("./output/", "P1-C4PFiles",  ".csv", sep = "")
write.table(
  t(c("File", "Snake", "Muscle", "Rater", "Mass(g)")),
  file = fC4P,
  append = FALSE,
  quote = TRUE,
  sep = ",",
  eol = "\n",
  na = "NA",
  dec = ".",
  row.names = FALSE,
  col.names = FALSE
)
#this for loop makes the large conglomerate file which is used by later code to make the subsetted files.
for (file in files) {
  y <- strsplit(file, "-")
  snake <- trimws(y[[1]][1])
  # get Snake Info
  sSpecies <- as.character(sinf$Species[which(sinf$Snake == snake)])
  sGenotype <-  as.character(sinf$Genotype[which(sinf$Snake == snake)])
  sMAMU <- as.character(sinf$MAMU[which(sinf$Snake == snake)])
  sCounty <- as.character(sinf$County[which(sinf$Snake == snake)])
  sLong <- as.character(sinf$Longitude[which(sinf$Snake == snake)])
  sLat <- as.character(sinf$Latitude[which(sinf$Snake == snake)])
  sSVLmm <- as.character(sinf$SVLmm[which(sinf$Snake == snake)])
 # sMusMassg <- as.character(sinf$MusMassg[which(sinf$Snake == snake)])
  sBodMassg <-  as.character(sinf$BodMassg[which(sinf$Snake == snake)])
  sSex <- as.character(sinf$Sex[which(sinf$Snake == snake)])
  #sExtraction <-  as.character(sinf$EXTRACTION[which(sinf$Snake == snake)])
  sDtExp <-  as.character(sinf$Date_Experimented[which(sinf$Snake == snake)])
  sDtColl <-  as.character(sinf$Date_Collected[which(sinf$Snake == snake)])
  sDInBet <-  as.character(sinf$Days_in_Between[which(sinf$Snake == snake)])
  # Get Muscle Mass
  muscle <- trimws(y[[1]][2])
  # Get Rater from Snake Info file for snake/musc combo
  rater <- as.character(sinf[which(sinf$Snake==snake), paste0("Rater..", muscle, ".")])

  muscMass <- smm[which(smm$SnakeID == snake), muscle] / 1000 #this factor is proportional to denomination of muscle mass (mg vs g)
  sMusMassg <- muscMass
  write.table(
    t(c(file, snake, muscle, rater, muscMass)),
    file = fC4P,
    append = TRUE,
    quote = TRUE,
    sep = ",",
    eol = "\n",
    na = "NA",
    dec = ".",
    row.names = FALSE,
    col.names = FALSE
  )
  ofdata <- paste("./output/", snake, "-", muscle, ".csv", sep = "")
  raw = read.csv(file)
  # Select all rows with stimulus > 1
  stmRows <- as.integer(rownames(raw[raw$Stimulus > 1.0,])) #could make > 2.0 to be consistent with J.A.Python
  if (length(stmRows) < 4) {
    cat("File: ", file, "; Stim Rows: ", length(stmRows), "\n")
    next
  }
  categories <- cutree(hclust(dist(stmRows)), k = 4)
  stmRows <- aggregate(stmRows, list(cats = categories), "max")[, 2]
  if (length(stmRows) < 4) {
    cat("File: ", file, "; Stim Rows: ", length(stmRows), "\n")
    next
  }
  
  # extract force-baseline for 1500 pts after end of stimulus; 
  i <- 0
  for (sr in stmRows) {
    i <- i + 1
    meanF = mean(raw$Force..g.[sr - 1000:sr])
    # muscWeight = smm[as.factor(snake),muscle] / 1000 (If this is no longer in play, what number is?)
    rspF <- (raw$Force..g.[stmRows[1]:(stmRows[1] + 1499)] - meanF) * (0.00980665 / muscMass)
    oFline <- c(
      sSpecies,
      sGenotype,
      sMAMU,
      sCounty,
      sLong,
      sLat,
      sSVLmm,
      sMusMassg,
      sBodMassg,
      sSex,
      sDtExp,
      sDtColl,
      sDInBet,
      i,
      snake,
      muscle,
      rater,
      muscMass,
      rspF
    )
    write.table(
      t(oFline),
      file = ofF,
      append = TRUE,
      quote = TRUE,
      sep = ",",
      eol = "\n",
      na = "NA",
      dec = ".",
      row.names = FALSE,
      col.names = FALSE
    )
    #  Low pass 200Hz filter -> pick every 50th entry and convert to Force / Sec (this is not a true Low pass; aliasing)
    #  Instead of Mani's downsampling, I'm going to use an actual first-derivative regime based on a smoothed spline
    #  This capitalizes on smooth.spline, predict() and diff(x)/diff(y)
    
    dt = (1:1500) / 10000
    spline <- smooth.spline(x= dt, y=rspF)
    prediction <- predict(spline)
    Force_Prime <- diff(rspF)/diff(dt)
    prediction <- predict(spline, deriv=1)
    rspF1d <- prediction$y
    #  rspF1d <- diff(rspF[1:30 * 50]) * 200 #need to change this to incorporate a cutoff Hz filter not downsampling filter
    oF1dline <- c(
      sSpecies,
      sGenotype,
      sMAMU,
      sCounty,
      sLong,
      sLat,
      sSVLmm,
      sMusMassg,
      sBodMassg,
      sSex,
      sDtExp,
      sDtColl,
      sDInBet,
      i,
      snake,
      muscle,
      rater,
      muscMass,
      rspF1d
    )
    write.table(
      t(oF1dline),
      file = ofF1d,
      append = TRUE,
      quote = TRUE,
      sep = ",",
      eol = "\n",
      na = "NA",
      dec = ".",
      row.names = FALSE,
      col.names = FALSE
    )
    maxF <- max(rspF)
    t100p <- min(which(rspF >= maxF)) #defines the minimum time at which the force is less than or equal to the maximum
    # convert all times to ms from tenth of a ms
    t10p <- min(which(rspF > 0.1 * maxF)) #defines the point where 10% of the peak is achieved during contraction
    t50p <- t100p + min(which(rspF[t100p:length(rspF)] <= 0.5 * maxF)) #defines the point where 50% of the peak is achieved during relaxation post peak
    maxF1d <- max(rspF1d)
    minF1d <- min(rspF1d)
    t1dmax <- min(which(rspF1d >= maxF1d)) # * 50
    t1dmin <- min(which(rspF1d <= minF1d)) # * 50
    sumLine = c(
      sSpecies,
      sGenotype,
      snake,
      muscle,
      rater,
      muscMass,
      sGenotype,
      i,
      meanF,
      maxF,
      t100p / 10,
      t10p / 10,
      t50p / 10,
      (t50p - t10p) / 10,
      maxF1d,
      minF1d,
      t1dmax / 10,
      t1dmin / 10,
      (t1dmin - t1dmax) / 10,
      sMAMU,
      sCounty,
      sLong,
      sLat,
      sSVLmm,
      sMusMassg,
      sBodMassg,
      sSex,
      sDtExp,
      sDtColl,
      sDInBet
    )
    write.table(
      t(sumLine),
      file = ofsum,
      append = TRUE,
      quote = TRUE,
      sep = ",",
      eol = "\n",
      na = "NA",
      dec = ".",
      row.names = FALSE,
      col.names = FALSE
    )
  }
} #End of above for loops (3)

df = read.csv("./output/c4p1-force.csv") #read the csv that we just made above
# soff <- off[order(off$Pulse,off$Snake.Muscle),]
# toff <- t(soff)
# write.csv(toff, "./output/c4p1-force-rpt.csv")
write.csv(t(df[order(df$Pulse, df$Snake, df$Muscle),]), "./output/c4p1-force-rpt.csv")
# Create reports by species and pulse
df <- df[order(df$Pulse, df$Species, df$Genotype, df$Snake, df$Muscle), ]
for (p in unique(df$Pulse)) {
  for (s in levels(df$Species)) {
    write.csv(df[which(df$Pulse == p &
                          df$Species == s),], paste0("./output/p1C4P-Force-P",
                                                     p, "-", s, ".csv"))
  }
}
#writing over obeject df based on the first derivative
#this didn't appear to be working even before I altered the first derivative function
df = read.csv("./output/c4p1-force1d.csv")
write.csv(t(df[order(df$Pulse, df$Snake, df$Muscle),]), "./output/c4p1-force1d-rpt.csv")
df <- df[order(df$Pulse, df$Species, df$Genotype, df$Snake, df$Muscle), ]
for (p in unique(df$Pulse)) {
  for (s in levels(df$Species[which(df$Pulse == p)])) {
    write.csv(df[which(df$Pulse == p &
                          df$Species == s),], paste0("./output/p1C4P-Force1d-P",
                                                     p, "-", s, ".csv"))
  }
}

# plot(rspF)
# plot(rspF1d,type="l")
# scatter.smooth(x=1:29, y=rspF1d)

